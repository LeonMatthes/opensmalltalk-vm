/* Automatically generated by
	VMPluginCodeGenerator * VMMaker.threaded-LM.3353 uuid: c431b328-cc1f-49b9-8427-ebfc50b37d7e
   from
	FilePlugin * VMMaker.threaded-LM.3353 uuid: c431b328-cc1f-49b9-8427-ebfc50b37d7e
 */
static char __buildInfo[] = "FilePlugin * VMMaker.threaded-LM.3353 uuid: c431b328-cc1f-49b9-8427-ebfc50b37d7e " __DATE__ ;
...
static sqInt primitiveFileReadPinningAndDisowning(void);
EXPORT(sqInt) primitiveFileReadThreaded(void);
...
static const char *moduleName = "FilePlugin * VMMaker.threaded-LM.3353 " INT_EXT;
static void * sCCPfn;
static void * sCDFfn;
static void * sCDPfn;
static void * sCGFTfn;
...
#  else // COGMTVM
	...
	count1 = positiveMachineIntegerValueOf(stackValue(0));
	startIndex1 = positiveMachineIntegerValueOf(stackValue(1));
	array1 = stackValue(2);
	/* begin fileValueOf: */
	...
	slotSize1 = slotSizeOf(array1);
	if (!((startIndex1 >= 1)
		 && (((startIndex1 + count1) - 1) <= slotSize1))) {
		primitiveFailFor(PrimErrBadIndex);
		goto l4;
	}
	elementSize1 = (slotSize1 == 0
		? 1
		: (byteSizeOf(array1)) / slotSize1);
	bytesRead1 = sqFileReadIntoAt(file1, count1 * elementSize1, ((char *) (firstIndexableField(array1))), (startIndex1 - 1) * elementSize1);
	if (!(failed())) {
		methodReturnInteger(bytesRead1 / elementSize1);
	}
...
#  else // SPURVM
	...
	retryCount = 0;
	count2 = positive32BitValueOf(stackValue(0));
	startIndex2 = positive32BitValueOf(stackValue(1));
	while (1) {
		array2 = stackValue(2);
		/* begin fileValueOf: */
		objectPointer2 = stackValue(3);
		if (!((isBytes(objectPointer2))
			 && ((byteSizeOf(objectPointer2)) == (sizeof(SQFile))))) {
			primitiveFailFor(PrimErrBadArgument);
			file2 = null;
			goto l5;
		}
		file2 = firstIndexableField(objectPointer2);
	l5:	/* end fileValueOf: */;
		if ((failed())
		 || (!(isWordsOrBytes(array2)))) {
			primitiveFailFor(PrimErrBadArgument);
			goto l6;
		}
		elementSize2 = (isWords(array2)
			? 4
			: 1);
		if (!((startIndex2 >= 1)
			 && (((startIndex2 + count2) - 1) <= (slotSizeOf(array2))))) {
			primitiveFailFor(PrimErrBadIndex);
			goto l6;
		}
		bytesRead2 = sqFileReadIntoAt(file2, count2 * elementSize2, ((char *) (firstIndexableField(array2))), (startIndex2 - 1) * elementSize2);
	...
	if (!(failed())) {
		methodReturnInteger(bytesRead2 / elementSize2);
	}
...
static sqInt
primitiveFileReadPinningAndDisowning(void)
{
    sqInt array;
    sqInt bytesRead;
    size_t count;
	...
	/* FilePlugin>>#primitiveFileReadThreaded */
EXPORT(sqInt)
primitiveFileReadThreaded(void)
{
    sqInt bufferSize;
    SQFile *file;
    sqInt objectPointer;
	...
	file = firstIndexableField(objectPointer);
	l1:	/* end fileValueOf: */;
	readBuffer = stackValue(1);
	readSemaphoreIndex = stackIntegerValue(0);
	if ((failed())
	 || ((!(isWordsOrBytes(readBuffer)))
	 || (!(isPinned(readBuffer))))) {
		primitiveFailFor(PrimErrBadArgument);
	}
	bufferSize = byteSizeOf(readBuffer);
	returnValue = sqThreadedFileReadIntoAtSignalling(file, bufferSize, ((char *) (firstIndexableField(readBuffer))), 0, readSemaphoreIndex);
	if (!(failed())) {
...
void* FilePlugin_exports[][3] = {
	...
	{(void*)_m, "primitiveFileReadThreaded\000\001\000", (void*)primitiveFileReadThreaded},
...
EXPORT(signed short) primitiveFileReadThreadedMetadata = 0x100;
